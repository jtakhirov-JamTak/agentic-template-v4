...
## Found during the docs split (2026-09-08), not fixed there

- **Governance write-protection does not hold cross-repo, so `CLAUDE.md`'s
  "everywhere" claim is false.** `write_guard.py`'s `GOVERNANCE` tuple (`:30`)
  covers `.claude/`, `.githooks/` and `CLAUDE.md`, but it is registered as a
  *project* hook. An agent whose session root is a different repo — here
  `~/.claude` — edits this repo's governance files with nothing loaded to object.
  Evidence: this session wrote `.claude/agents/evaluator.md` and `CLAUDE.md` and
  no guard fired. `CLAUDE.md:155` says they are "closed to the agent on the write
  tools everywhere". Same class as A3 (project hooks skipped → fails open
  silently, and nothing says so). Fix is one of: narrow the claim to
